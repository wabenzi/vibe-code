...
count_lines() {
    ...
    if [ -n "$exclude_pattern" ]; then
        ...
        find "$PROJECT_ROOT" -name "$pattern" | xargs wc -l 2>/dev/null | sort -nr || echo "0 total"
    fi
}
...
if find "$PROJECT_ROOT/src" -name "*.ts" -print0 | xargs -0 wc -l > /tmp/src_wc 2>/dev/null; then
    SRC_OUTPUT=$(cat /tmp/src_wc | sort -nr)
...
if find "$PROJECT_ROOT/test" -name "*.ts" -print0 | xargs -0 wc -l > /tmp/test_wc 2>/dev/null; then
    TEST_OUTPUT=$(cat /tmp/test_wc | sort -nr)
...
if find "$PROJECT_ROOT/infrastructure" -name "*.ts" -print0 | xargs -0 wc -l > /tmp/infra_wc 2>/dev/null; then
    INFRA_OUTPUT=$(cat /tmp/infra_wc | sort -nr)
...
if find "$PROJECT_ROOT" -maxdepth 2 -name "*.js" -o -name "*.mjs" | grep -v node_modules | grep -v cdk.out | xargs wc -l > /tmp/config_wc 2>/dev/null; then
    CONFIG_JS_OUTPUT=$(cat /tmp/config_wc | sort -nr)
...
if find "$PROJECT_ROOT/scripts" -name "*.ts" -o -name "*.js" -o -name "*.mjs" 2>/dev/null | xargs wc -l > /tmp/scripts_wc 2>/dev/null; then
    SCRIPTS_OUTPUT=$(cat /tmp/scripts_wc | sort -nr)
